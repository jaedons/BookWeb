...
public class DateUtil {
    ...
    private static DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern(dateParttern);
    ...
    public static Date dayOfWeek(Date date,int dayOfWeek) throws ParseException {
        // 日期格式转换
        String dateStr = dateToStr(date,dateParttern);
        // 计算是周几
        ...
        // Date 数据格式 返回
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(dateParttern);
        return strToDate(targetDay.format(formatter),dateParttern);
    }
    ...
    public static void main(String[] args) {
        try {
            Date day = dayOfWeek(new Date(),7);
            System.out.println(dateToStr(day));
